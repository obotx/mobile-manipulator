...
class ParallelRobot:
    ...
    def __init__(self, path: str):
        ...
        self.data.ctrl[self.actuator_ids] = [0, 0,  0, 0, 0]
    ...
    def control_base(self, target_arm_id, alpha=0.7):
        # --- PID gains ---
        k_p = 3.0
        k_i = 0.02
        k_d = 0.3       
    ...
    def ik_solution(
        ...
        target = ee_base + err_base
        
        if hasattr(self, "_last_ik_target"):
            if np.linalg.norm(target - self._last_ik_target) < recompute_thresh and hasattr(self, "h1"):
                return
        ...
        bounds = [
            (float(h_bounds[0,0]), float(h_bounds[0,1])),  # h1
            (float(h_bounds[1,0]), float(h_bounds[1,1])),  # h2
            (float(theta_bounds[0]), float(theta_bounds[1]))  # theta
        ]
    ...
    def control_base_arm(self, target_angle, kp=4, ki=0.05, kd=3):
    ...
    def on_key(self, window, key, scancode, action, mods):
        ...
        # jaw control (finger actuators)
        if key in (glfw.KEY_Z, glfw.KEY_X):
            for act_id in self.gripper_act_ids[0:3]:  # exclude last 2 (side joints)
                current_val = self.data.ctrl[act_id]

                if key == glfw.KEY_Z:  # Close gripper
                    new_val = np.clip(
                        current_val + self.GRIPPER_INCREMENT,
                        self.GRIPPER_OPEN_POS,
                        self.GRIPPER_CLOSED_POS
                    )
                elif key == glfw.KEY_X:  # Open gripper
                    new_val = np.clip(
                        current_val - self.GRIPPER_INCREMENT,
                        self.GRIPPER_OPEN_POS,
                        self.GRIPPER_CLOSED_POS
        ...
        # side control (last two actuators)
        if key in (glfw.KEY_C, glfw.KEY_V):
            ...
            if key == glfw.KEY_C:  # Move outward
...
if __name__ == "__main__":
    sim = ParallelRobot("arm_obotx_with_mobile_base.xml")
    sim.run()