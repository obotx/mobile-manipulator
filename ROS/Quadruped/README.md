...
├── mm_nav2               
│   ├── launch                    # run nav2 with octomap
│   ├── params                    # nav2 config
│   └── src                       # nav2 set goal main code
...
ros2 run mm_bringup sim_moveit.sh
```
...
### MoveIt Pick and Place (with MoveIt Task Constructor and Stretch Kinematic Solver)

```bash
# From workspace root: /mobile-manipulator/ROS/Quadruped
# Run pick-and-place Moveit Task Constructor demo pipeline
ros2 run mm_bringup mtc_pick_place.sh
```
...
### MoveIt Pick and Place with Nav2 
...
# Terminal 1
ros2 run mm_bringup sim_pick_place_moveit_nav2.sh

# Terminal 2 (wait after the bringup fully load)
ros2 run mm_moveit_demos pick_place.py
```
...
Octomap generated by using [gazebo_map_creator](https://github.com/arshadlab/gazebo_map_creator/tree/master) in **ROS2 Humble + Gazebo Ignition** different workspace. Then, we add those Octomap to MoveIt planning scene, you can check [here](src/mm_mapping/src/moveit_octomap_handler.py).  
