...
int main(int argc, char * argv[])
{
    ...
    auto node = std::make_shared<rclcpp::Node>("auto_arm_selector", options);
    ...
    std::string model_frame = current_state->getRobotModel()->getModelFrame();
    RCLCPP_INFO(logger, "MoveIt model frame = %s", model_frame.c_str());

    // END EFFECTORS
    const std::string left_tip = "obotx_left_tool0";
    const std::string right_tip = "obotx_right_tool0";
    ...
    RCLCPP_INFO(logger, "Distance LEFT  = %.3f", dist_left);
    RCLCPP_INFO(logger, "Distance RIGHT = %.3f", dist_right);
    ...
    Eigen::Isometry3d current_ee_tf = current_state->getGlobalLinkTransform(tip_link);
    Eigen::Quaterniond q(current_ee_tf.rotation());
    ...
    arm_group.setPoseTarget(target_pose, tip_link);
