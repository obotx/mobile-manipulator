...
ros2 launch mm_gazebo morph_i.gazebo.launch.py use_rviz:=false world_file:=pick_place.sdf &
PIDS+=($!)
sleep 10

echo "[INFO] Launching MoveIt..."
ros2 launch mm_moveit_config move_group_octomap.launch.py&
PIDS+=($!)
sleep 10

echo "[INFO] Launching Nav2..."
ros2 launch mm_nav2 octomap_nav.launch.py use_rviz:=false use_moveit:=true &
PIDS+=($!)
wait