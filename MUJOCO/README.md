# MuJoCo Version: MORPH Series – Mobile Omni Robotic Platform with Hands
...
All commands below should be executed from the `Quadruped` directory:
```bash
...
uv run src/examples/tracking_publisher.py \
  --calibration src/camera_calibration/calibration_results/cam_calib_\(10x7\)_22.0mm_1920x1080.npz \
  --use-bpf \
  --width 1920 \
  --height 1080
```

In **Terminal 2**, launch the MuJoCo simulation which subscribes to the landmark topics to control the robot.
```bash
